...
// Desc      Import data from a excel file
// Route     POST /todos/importFromExcelQueue
// Access    PRIVATE
export const importFromExcelFileQueue = async (req: Request, res: Response) => {
  try {
    if (!req.file) throw new Error("File not found");
    let userId = req.user.id;
    let file = req.file;
    ...
    let job = await todoServices.importFromExcelFileQueue(
      userId,
      file,
      sheetNum
    );
...
// Desc      Export data to excel file
// Route     GET /todos/exportToExcelQueue
// Access    PRIVATE
export const exportToExcelQueue = async (req: Request, res: Response) => {
  try {
    const userId = req.user.id;
    let { limit, page }: any = req.query;
    ...
    // import-export
    // id, type, user, filename, status => waiting, active, complete, error
    // socket - ban k

    await todoServices.exportToExcelFileQueue(userId, page, limit);

    res.status(200).json({ message: "File is proccessing" });
  } catch (errors: Error | any) {
    if (errors instanceof Error) {
      res.status(500).json({ error: errors.message });
    } else {
